...
void EventEntry(Event* (&d), int& n)
{
	...
	d = new Event[n]; //выделение памяти для массива данных
	...
	for (int i = 0; i < n; i++)
	{
		...
		d[i].EventEntry(Name, date); //запись данных в массив
...
void AddEvent(Event* (&d), int& n)
{
	...
	Event* buf = new Event[n]; //временный массив
	int size = n, 
	new_size = ++n;

	Copy(buf, d, size);

	d = new Event[new_size]; //выделение памяти с размером +1
...
void SaveEvent(Event* d, int n, string filename)
{
	...
	if (record)
	{
		...
		for (int i = 0; i < n; i++)
		{
			record << " " << d[i].GetName().name << endl;
			record << d[i].GetDate().year << "/" << d[i].GetDate().month <<"/"<< d[i].GetDate().year <<" "<< d[i].GetDate().hour <<":"<< d[i].GetDate().min << endl;

			if (i < n - 1) //проверка: является ли элемент последним
				record << endl;
		}
